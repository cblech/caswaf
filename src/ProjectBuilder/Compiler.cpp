...
void Compiler::actionCompileHTML() {
	...
	scanAllFilesAndUpdateCache();
	compileParts();

	ofstream ofsDotH((generatedPath / "html.generated.h").string());
	ofstream ofsDotCpp((generatedPath / "html.generated.cpp").string());
	//ofstream ofs((generatedPath / "html.generated.h").string());

	ofsDotH << "// Generated by Project Builder" << endl;
	ofsDotH << "//" << endl;
	ofsDotH << "//" << endl;
	ofsDotH << "// #####################" << endl;
	ofsDotH << "// ##  DO NOT MODIFY  ##" << endl;
	ofsDotH << "// #####################" << endl;
	ofsDotH << "//" << endl;

	ofsDotCpp << "// Generated by Project Builder" << endl;
	ofsDotCpp << "//" << endl;
	ofsDotCpp << "//" << endl;
	ofsDotCpp << "// #####################" << endl;
	ofsDotCpp << "// ##  DO NOT MODIFY  ##" << endl;
	ofsDotCpp << "// #####################" << endl;
	ofsDotCpp << "//" << endl;

	ofsDotCpp << "#include \"html.generated.h\"" << endl << endl;

	for (auto pair : projectStructure.getPartsForBuild()) {
		path p = pair.second.originalPath;
		path destpath = (generatedPath / p.lexically_relative(resoucesPath)).string() + ".h";
		ofsDotH << "#include \"" << destpath.lexically_relative(generatedPath).string() << "\"" << endl;
		trace << "#include \"" << destpath.lexically_relative(generatedPath).string() << "\"";

		ofsDotCpp << "R::Part::"<<pair.second.name<<"* R::Part::" << pair.second.name << "::instance = new R::Part::" << pair.second.name << "();" << endl;
	}

	ofsDotH << endl << " namespace R::PluginPoints{" << endl;

	for(auto pp:projectStructure.getPluginPoints())
	{
		ofsDotH << "\textern int " << pp.name << ";" << endl;
		ofsDotCpp << "int R::PluginPoints::" << pp.name << " = " << pp.id << ";" << endl;
	}
...
void Compiler::scanAllFilesAndUpdateCache(path p)
{
	try {
		...
		if (fs::is_regular_file(p)) {
			if (p.extension() == ".html") {
				//####
				// Interpreting File in p
				//####

				//extract the Tecnical Partname
				std::string partName = p.filename().string().substr(0, p.filename().string().size() - 5);


				//debug
				debug << "Interpreting: " << p;
				...
				//Add the part to the project structure and get the id
				int partID = projectStructure.addPart(partName, p);

				int counter = 0;
				for (auto token : tokens)
				{
					projectStructure.addTokenToPart(++counter, partID, token);
				}
		...
		//p is directory -> scanning all files (and directories) in it.
		else {
			fs::directory_iterator end_itr;

			// cycle through the directory
			for (fs::directory_iterator itr(p); itr != end_itr; ++itr) {
				scanAllFilesAndUpdateCache(itr->path());
			}
		}
	}
...
void Compiler::compileParts()
{
	//iterate through all parts to generate the .h files
	for (auto partPair : projectStructure.getPartsForBuild())
	{
		auto part = partPair.second;

		debug << "Compiling: " << part.name;

		path destPath = (generatedPath / part.originalPath.lexically_relative(resoucesPath)).string() + ".h";
		...
		ofs << "#pragma once\n#include \"engine/part/StaticPart.h\"\n\nnamespace R::Part{\n\nclass " << part.name << " :public StaticPart\n{\npublic:\n";


		ofs << "\t" << part.name << "()\n\t{\n";

		for (htmlToken token : part.token) {
			ofs << "\t\tfragmentList.push_back(Fragment( ";

			switch (token.type) {
			case htmlTokenType::html:

				ofs << strToIntList(token.text) ;
		...
		//ofs << "\n\t" << part.name << " addSubpart(int connectionPoint,  Part * addedPart){\n\t\tStaticPart::addSubpart(static_cast<int>(connectionPoint), addedPart);\n\t\treturn *this;\n\t}" << endl;

		//ofs << "\t" << part.name << " * make()\n\t{\n\t\treturn new " << part.name << "(*this);\n\t}\n";
		
		ofs << "\tstatic " << part.name << " * getInstance()\n\t{\n\t\treturn instance;\n\t}\n";

		ofs << "private:\n\tstatic " << part.name << " * instance;" << endl;
